...
import { getModuleQuiz, startQuizAttempt, submitQuizAttempt, getQuizResults } from '../services/studentService';

const QuizComponent = ({ moduleId, onQuizComplete }) => {
  ...
  const loadQuiz = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getModuleQuiz(moduleId);
      if (response.data.quiz) {
        setQuiz(response.data.quiz);
        // Set timer to 30 minutes (1800 seconds) for timed quizzes
        setTimeLeft(1800);
      } else {
        setQuiz(null);
      }
  ...
  const handleTimeUp = useCallback(async () => {
    setIsActive(false);
    setWarning('Time is up! Your quiz has been auto-submitted.');
    if (attemptId) {
          ...
          <div className="card-body">
            <div className="row">
              <div className="col-md-8">
                ...
                <p className="mb-2"><strong>Time Limit:</strong> 30 minutes</p>
                <p className="text-muted small">
                  Once you start the quiz, the timer will begin and you must complete it within the time limit.
                </p>
              </div>
        ...
        <div className="card-header">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div className="timer">
              ...
              <span className={`badge ${timeLeft < 300 ? 'bg-danger' : 'bg-primary'}`}>
                {formatTime(timeLeft)}
              </span>
            </div>
          </div>
